...
def get_num_layer_for_vit_and_llama(var_name, vit_num_max_layer, llama_num_max_layer):
...
def create_optimizer(self):
    ...
    try:  # for stage2 model
        ...
        llama_num_layers = opt_model.config.llama_config.num_hidden_layers + 2
    except:  # for stage3 model
        vit_num_layers = opt_model.llama.config.vision_config.num_hidden_layers + 2
        llama_num_layers = opt_model.llama.config.llama_config.num_hidden_layers + 2
    print('vit_num_layers:', vit_num_layers)
    print('llama_num_layers:', llama_num_layers)

    vit_layer_decay_rate = float(os.getenv('VIT_LAYER_DECAY_RATE', 1.0))
    llama_layer_decay_rate = float(os.getenv('llama_LAYER_DECAY_RATE', 1.0))
    print('vit_layer_decay_rate:', vit_layer_decay_rate)
    print('llama_layer_decay_rate:', llama_layer_decay_rate)
    ...
    for name, param in opt_model.named_parameters():
        ...
        layer_id = get_num_layer_for_vit_and_llama(name, vit_num_layers, llama_num_layers)
        group_name = '%s_layer_%d_%s' % (cls, layer_id, group_name)
        if group_name not in parameter_groups:
            if cls == 'vit':
                scale = vit_layer_decay_rate ** (vit_num_layers - layer_id - 1)
            else:
                scale = llama_layer_decay_rate ** (llama_num_layers - layer_id - 1)
            scale = min(1.0, scale)
